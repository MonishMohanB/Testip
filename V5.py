    ...
    try:
        ...
        for idx, row in df.iterrows():
            try:
                ...
                # Filter out empty list entries from validated_optional
                filtered_optional = {k: v for k, v in validated_optional.items() if v or isinstance(v, str)}
                ...
                # Move selected keys into model_setting
                model_setting_keys = ['time_var', 'time_series_vars', 'categorical_vars', 'scenario_var']
                model_setting = {k: filtered_optional[k] for k in model_setting_keys if k in filtered_optional}
                merged_config['model_setting'] = model_setting

                # Add remaining optional settings
                for k, v in filtered_optional.items():
                    if k not in model_setting_keys:
                        merged_config[k] = v
